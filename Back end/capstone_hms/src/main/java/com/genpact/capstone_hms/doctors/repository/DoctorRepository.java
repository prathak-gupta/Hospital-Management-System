...
public class DoctorRepository {
    ...
    public void createDoctor(Doctor doctor) {
        ...
        try {
            doctorJdbc.update(sql, doctor.getFirstName(), doctor.getLastName(), doctor.getSpecialization(), doctor.getPhoneNumber(), doctor.getEmail(), doctor.getDepartment(), doctor.getQualification(), doctor.getYearsOfExperience(), doctor.getCharges());
        } catch (Exception e) {
    ...
    // Get Latest Doctor
    public Doctor getLatestDoctor() {
        String sql = "SELECT * FROM Doctors ORDER BY Registration_date DESC LIMIT 1";
        try {
            return doctorJdbc.queryForObject(sql, doctorRowMapper);
        } catch (Exception e) {
            System.err.println("Error: " + e.getMessage());
            e.printStackTrace();
            return null;
